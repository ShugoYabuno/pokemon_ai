...
from .enum_classes import TemporaryStatusAilment, EternalStatusAilment, SingleBattleActions
...
class PokemonState:
    ...
    eternal_status_ailments: list[EternalStatusAilment]
    temporary_status_ailments: list[TemporaryStatusAilment]
...
class SingleBattle:
    ...
    playing_pokemons: list[PokemonState, PokemonState]
    ...
    def __init__(self, _party0: list[tuple[Pokemon, Pokemon, Pokemon]], _party1: tuple[Pokemon, Pokemon, Pokemon]) -> None:
        ...
        self.playing_pokemons = [self.parties[0][0], self.parties[1][0]]
        self.information = ((), ())
    ...
    def _switch_pokemon(self, _player: 0 | 1, _action: 5 | 6):
        benches = []
        for _state in self.parties[_player]:
            if _state.pokemon.name != self.playing_pokemons[_player].pokemon.name:
                benches.append(_state)

        if _action == 5:
            self.playing_pokemons[_player] = benches[0]
        else:
            self.playing_pokemons[_player] = benches[1]

    def _sorted_players(self):
        return sorted(
            [0, 1], key=lambda x: self.playing_pokemons[x].get_speed(), reverse=True)
    ...
    def advance_turn(self):
        ...
        if self.actions[0] in [5, 6] or self.actions[1] in [5, 6]:
